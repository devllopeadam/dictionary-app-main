  ...
  useEffect(() => {
    if (data) {
      if (data[0].phonetics.length > 0) {
        if (data[0].phonetics.some((e) => e.audio !== "")) {
          data[0].phonetics.filter((ele) => {
            if (ele.audio !== "") {
              setPhonetic(ele);
              setSound(ele.audio);
            }
        ...
        <div
          ...
          {error && (
            <motion.div
              ...
              className="text-[14px] text-red absolute -bottom-[30px] left-0 font-bold">{`Can't be empty`}</motion.div>
          )}
        </div>
        {!noDefinition ? (
          <motion.div
